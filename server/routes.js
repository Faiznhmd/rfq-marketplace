...
import { idSchema, rfqSchema, quotationSchema, searchSchema } from '../shared/validation.js';
...
const rfqColumns = `r.id, r.buyer_id AS "buyerId", r.product_name AS "productName", r.description,
  ...
  r.created_at AS "createdAt", (r.deadline > (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date) AS "isOpen"`;
const quoteColumns = `q.id, q.rfq_id AS "rfqId", q.supplier_id AS "supplierId", q.price::text AS price,
  q.delivery_time AS "deliveryTime", q.message, q.created_at AS "createdAt"`;
...
export function marketplaceRoutes(db, production) {
  ...
  router.get('/rfqs', supplier, async (req, res) => {
    ...
    const { rows } = await db.query(
      `SELECT ${rfqColumns} FROM rfqs r
      WHERE r.deadline > (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date
      AND (r.product_name ILIKE $1 OR r.description ILIKE $1) AND r.delivery_location ILIKE $2`,
      [`%${escapeLike(q)}%`, `%${escapeLike(location)}%`],
    );
    res.json({ rfqs: rows });
  });
  ...
  router.get('/rfqs/:id/quotations', buyer, async (req, res) => {
    ...
    const { rows } = await db.query(
      ...
      JOIN users u ON u.id=q.supplier_id WHERE q.rfq_id=$1`,
      [req.params.id],
    );
  ...
  router.post('/rfqs/:id/quotations', supplier, async (req, res) => {
    const rfq = await getRfq(req.params.id, req.user);
    if (!rfq.isOpen)
      throw new HttpError(409, 'The deadline has passed. This RFQ no longer accepts quotations.');
    const data = parse(quotationSchema, req.body);
    try {
      // Check the deadline again in the write itself, so an expired RFQ cannot receive a quote.
      const { rows } = await db.query(
        `INSERT INTO quotations (id,rfq_id,supplier_id,price,delivery_time,message)
        SELECT $1,r.id,$3,$4,$5,$6 FROM rfqs r WHERE r.id=$2 AND r.deadline > (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date
        RETURNING id, rfq_id AS "rfqId", supplier_id AS "supplierId", price::text AS price, delivery_time AS "deliveryTime", message, created_at AS "createdAt"`,
